// (Function) Pass an input audio signal through a flanger filter
#include "effects.h"

float flanger(float in, float tau, int fE, int fS){
    // INPUT ARGUMENTS:
    ...
    const int len = (int)(tau*fS); // Number of samples in each input buffer (>0, unitless)
