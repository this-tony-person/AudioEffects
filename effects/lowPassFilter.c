// (Function) Pass an input audio signal through a first-order low-pass filter
float lowPassFilter(float in, float tau, float Ts) {
    // INPUT ARGUMENTS:
    ...
    out = 1/(2*tau+Ts)*(out0*(2*tau-Ts) + (in+in0)*Ts);
